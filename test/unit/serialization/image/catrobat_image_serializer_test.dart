...
void main() {
  group('Version 1', () {
    test('Test CatrobatImage serialization with 1 path', () {
      Iterable<Command> commands = DummyCommandFactory.createCommandList(
        1,
        version: Version.v1,
      );
    ...
    test('Test CatrobatImage.fromBytes legacy binary fallback', () {
      final legacyBytes = Uint8List.fromList([
        0x11, // SetDimensionCommand class ID (15 + 2)
        0x64, 0x00, 0x00, 0x00, // width = 100
        0xC8, 0x00, 0x00, 0x00, // height = 200
        0x00, 0x00, 0x00, 0x00, // commands size = 0
      ]);

      final image = CatrobatImage.fromBytes(legacyBytes);
      expect(image.width, equals(100));
      expect(image.height, equals(200));
      expect(image.commands, isEmpty);
      expect(image.magicValue, equals('CATROBAT'));
    });
  });
}
